                            ...
                            <div id="menu" class="container mx-auto px-4 lg:pt-24 lg:pb-64">
                                <div class="flex flex-wrap text-center justify-center">
                                    <div class="w-full  px-6">
                                        <h2 class="text-4xl font-semibold text-black">Pedido #{{$pedido->id}}</h2>
                                        <p class="text-lg mt-4 mb-4 text-gray-900">
                                            Su Pedido ha sido registrado. Al pasar por el muestre este Código QR:
                                        </p>
                                        {{-- <img class="inline-block align-middle mb-2 border-solid border-coyos-lightyellow border-4 rounded-2xl " src="{!!$message->embedData(QRCode::format('svg')->generate($pedido->id), 'QrCode.png', 'storage/image/qrpedidos')!!}"> --}}
                                        <img class="inline-block align-middle mb-2 border-solid border-coyos-lightyellow border-4 rounded-2xl " src="storage/images/qrpedidos/qr_pedido{{$pedido->id}}.svg" alt="">
                                ...
                                <div class="row flex justify-center my-2">
                                    <div class="flex max-w-max-content col-span-2 md:col-span-4 xl:col-span-4">
                                        ...
                                        <h4 class="rounded bg-coyos-lightblue mr-3 px-2">{{$pedido->fecha}}</h4>
                                        <h3 class="overflow-x-auto pr-5 font-semibold text-black"> a las </h3>
                                        <h4 class="rounded bg-coyos-lightblue mr-3 px-2">{{$pedido->hora}}</h4>
                                        {{-- sucursal --}}
                                        <h4 class="overflow-x-auto pr-5 font-semibold text-black"> en Sucursal :</h4>
                                        <h4 class="rounded bg-coyos-lightblue mr-3 px-2">{{ucwords(str_replace("_", " ", $pedido->sucursal))}}</h4>
                                        <hr>
                                        <hr>
                                ...
                                <div class="flex flex-wrap w-80  min-w-full mt-12 justify-center">
                                    {{-- {{dd($pedido->productosDePedido)}} --}}
                                @foreach ($pedido->productosDePedido as $prod)
