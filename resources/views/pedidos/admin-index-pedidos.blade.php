                                ...
                                <td class="px-2 border-solid border-r-4 border-coyos-darkblue" ><p class="{{$clases}}">{{$pedido->status}}</p></td>
                                    ...
                                    <td class="px-2 text-center border-solid border-r-4 border-coyos-darkblue">
                                        <form action="{{ route('pedidos.admin-confirma-pedido-update', $pedido->id) }}" method="POST">
                                            @method('PUT')
                                            @csrf
                                            <button type="submit" name="confirma" id="confirma{{$pedido->id}}" class="text-center align-middle shadow-outline-indigo  hover:bg-green-400 mx-2 hover:text-coyos-lightblue bg-coyos-darkblue text-green-400 rounded text-xl px-2 py-2 text-bold">¿Confirmar?</button>
                                        </form>
                                    </td>
                                    <td class="px-2 text-center border-solid border-r-4 border-coyos-darkblue">
                                        <form action="{{route('pedidos.admin-cancela-pedido-update',$pedido->id)}}" method="POST">
                                            @method('PUT')
                                            @csrf
                                            <button name="{{$pedido->id}}" id="cancela{{$pedido->id}}"  class="text-center align-middle shadow-outline-indigo  hover:bg-coyos-lightblue mx-2 hover:text-coyos-darkpink bg-coyos-darkpink text-coyos-lightblue rounded text-xl px-2 py-2 text-bold">¿Cancelar?</button>
                                        </form>
                                    </td>
